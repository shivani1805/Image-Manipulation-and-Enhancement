...
/**
 * This class represents the blur image manipulation function.
 */
public class Blur implements CommandController {
    ...
    if (listOfCommands.length == argumentCountSplit) {
      if (this.listOfCommands[splitKey].equalsIgnoreCase("split")) {
        if (this.listOfCommands[percentValue] != null) {
          model.blurCommand(this.listOfCommands[parentImageIndex],
  ...
  public void checkArguments(String[] commands) throws IllegalArgumentException {
    if (this.listOfCommands.length != argumentCountSplit
      ...
      throw new IllegalArgumentException("Incorrect command tokens provided. "
              + "Expected count: " + argumentCountSplit + " or " + argumentCountNonSplit
              + " arguments, but received " + listOfCommands.length + " arguments.");
    }
  }
